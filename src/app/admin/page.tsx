...
export default function AdminDashboard() {
  ...
  const formatStock = (stockInBase: number, dimension: Dimension, unit: Unit) => {
    if (dimension === 'WEIGHT') {
      if (unit === 'kg') return `${(stockInBase / 1000).toFixed(4)} kg`;
      return `${stockInBase.toFixed(2)} g`;
    }
    if (dimension === 'VOLUME') {
      if (unit === 'L') return `${(stockInBase / 1000).toFixed(4)} L`;
    ...
    <div className="dashboard-layout">
      ...
      <header className="dashboard-nav">
        <div className="logo-container" style={{ marginBottom: 0 }}>
          ...
          <div className="logo-text" style={{ fontSize: '1.25rem' }}>AasaMedChem <span style={{ fontSize: '0.8rem', color: 'var(--primary)', verticalAlign: 'super' }}>Admin</span></div>
        </div>
        ...
        <div className="tabs-container">
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <ListOrdered size={18} />
              Incoming Orders & Quotations ({orders.filter(o => o.status === 'PENDING').length} Pending)
            </span>
          </button>
          <button 
        ...
        {activeTab === 'orders' ? (
          ...
          <div className="glass-panel" style={{ width: '100%' }}>
            <div className="panel-header">
              <h3>Order Verification Dashboard</h3>
            </div>
                    ...
                    <div key={order.id} className="glass-panel order-card">
                      <div className="order-header">
                        <div className="order-meta">
                          ...
                          <span>Seller: <span style={{ color: 'white', fontWeight: 600 }}>{order.user.name}</span> ({order.user.email})</span>
                          <span>Date: {new Date(order.createdAt).toLocaleString()}</span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                          <span style={{ fontWeight: 800, fontSize: '1.2rem', color: 'white' }}>₹{order.totalPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                            ...
                            {order.items.map(item => {
                              ...
                              return (
                                <tr key={item.id} style={{ borderBottom: '1px solid var(--border-glass)' }}>
                                    ...
                                    {isConverted && (
                                      <div className="conversion-breakdown" style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                        ...
                                        <span>
                                          ...
                                          Pricing rate: ₹{item.priceAtOrder} per {item.priceUnitAtOrder} (calculated rate of ₹{(item.priceUnitAtOrder === 'kg' || item.priceUnitAtOrder === 'L' ? item.priceAtOrder / 1000 : item.priceAtOrder).toFixed(6)} per base {baseUnit}).
                                        </span>
                                      </div>
                                    )}
                                  </td>
                                  ...
                                  <td style={{ textAlign: 'right', fontWeight: 700, color: 'white' }}>₹{item.calculatedPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                                </tr>
                              );
                            })}
                          </tbody>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
            <div className="glass-panel">
              <div className="panel-header">
                <h3>Stock Alerts & Inventory Levels</h3>
