...
export default function SellerDashboard() {
  ...
  const formatStock = (stockInBase: number, dimension: Dimension, displayUnit: Unit) => {
    if (dimension === 'WEIGHT') {
      if (displayUnit === 'kg') return `${(stockInBase / 1000).toFixed(4)} kg`;
      return `${stockInBase.toFixed(2)} g`;
    }
    if (dimension === 'VOLUME') {
      if (displayUnit === 'L') return `${(stockInBase / 1000).toFixed(4)} L`;
      return `${stockInBase.toFixed(2)} mL`;
    }
    return `${stockInBase} items`;
  };
  ...
  const addToCart = () => {
    ...
    if (reqBase > selectedProduct.stock) {
    ...
    <div className="dashboard-layout">
      ...
      <header className="dashboard-nav">
        <div className="logo-container" style={{ marginBottom: 0 }}>
          ...
          <div className="logo-text" style={{ fontSize: '1.25rem' }}>AasaMedChem</div>
        </div>
          ...
          <div className="grid-three-two">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div className="glass-panel" style={{ padding: '20px' }}>
                <h3 style={{ marginBottom: '15px' }}>Search Inventory</h3>
              ...
              <div className="grid-cards">
                {filteredProducts.length === 0 ? (
                  ...
                  filteredProducts.map(product => {
                    const lowStock = product.stock <= product.price * 0; // Temp check placeholder, or actual alert check:
                    // If product.stock <= minStockAlert:
                    const isLowStock = product.stock <= 0; // Out of stock
                    ...
                    return (
                      <div key={product.id} className="glass-panel product-card">
                        ...
                        <div className="product-meta-row">
                          <div>
                            <span className="product-price-label">₹{product.price.toLocaleString('en-IN')}</span>
                    ...
                    <div className="conversion-breakdown">
                      ...
                      {orderUnit !== selectedProduct.priceUnit ? (
                        <>
                          ...
                          - Configured pricing rate: ₹{selectedProduct.price} per {selectedProduct.priceUnit} (₹{selectedProduct.priceUnit === 'kg' || selectedProduct.priceUnit === 'L' ? selectedProduct.price / 1000 : selectedProduct.price} per base unit)
                        </>
                      ) : (
                  ...
                  {cart.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="cart-summary">
                        ...
                        >
                          {submittingOrder ? 'Placing Quotation...' : 'Submit Quotation Order'}
                        </button>
                      </div>
                    </>
                  )}
                    ...
                    <div key={order.id} className="glass-panel order-card">
                      <div className="order-header">
                        <div className="order-meta">
                          <span>Order ID: <span className="order-id">{order.id.slice(0, 8)}...</span></span>
                            ...
                            {order.items.map(item => {
                              ...
                              return (
                                <tr key={item.id}>
                                  ...
                                  <td style={{ textAlign: 'right', fontWeight: 600 }}>₹{item.calculatedPrice.toFixed(2)}</td>
                                </tr>
                              );
                            })}
                          </tbody>
